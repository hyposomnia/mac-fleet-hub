...
- 排队消息条移除冗长的“服务器已保存 / 当前任务结束后发送”说明，右侧收敛为编辑、引导、删除三个纯图标；编辑会先安全取消原排队项，再把文字与附件完整回填到输入框。PWA 外壳缓存升级到 v139。
- 聊天输入框的附件选择不再限定图片：保留图片预览，并为其它文件显示文件名、大小与类型图标；fleet-agent 保存原始文件名/MIME，并把模型可读取的本地附件路径传给 ChatGPT 或 DeepSeek，模型不支持的类型则由模型自行忽略或说明。PWA 外壳缓存升级到 v138。
- 会话列表移除容易失真的“几分钟前 / 几小时前”，改为标题前的状态圆点，区分等待回复、进行中、未读、已读与归档；已读游标保存在浏览器本地，打开会话后立即清除未读状态。
- 输入框发送按钮、回车与 Command/Ctrl+Enter 统一提交到服务端的“下一轮”队列，不再默认引导当前运行中的任务；只有用户明确点击“引导当前任务”才切换为 steer。
...
- 修复 **网关不可达时被误读成「一台 Mac 都没入网」**：`refreshNodes()` 原本在 `/api/nodes.json` 非 2xx 时静默 `return`、`fetch` 抛错时 `catch (_) {}` 空吞，于是网关宕机和「车队本来就是空的」在界面上完全同形——用户会去排查终端/设备，而真正的问题在网关那一层。现在失败路径记入 `state.gatewayDown`（带 HTTP 状态码/网络错误原因便于排障），左栏空态改为 danger 色系的「⚠ 无法连接服务器 / 这不代表没有 Mac 入网」并给出可点的**重试**；恢复可达后自动清除故障态。移动端设备选择器与文件模式的同类空态一并区分。新增 `gateway_down.test.mjs`（11 例）并纳入 `scripts/verify.sh`。
...
- 会话删除成功后才释放聊天缓存，失败时保留当前聊天；旧 agent 未声明菜单能力时不展示不可执行的操作。PWA 外壳更新至 v126，并清除后端错误提示中的旧 Codex 名称及终端回退提示。
...
- 修复 `mac/migrate-existing-client-to-shared.sh` 的 UAT 探针在「刚换完 app-server」窗口里一枪判死：迁移过程中 fleet-agent 的 WS 客户端正在重建，此时 `/api/chat/skills`、`/api/sessions`、`/api/chat/resume` 会回 503（`appserver_unavailable` / `appserver_recovered` / `agent_restarting`，都是稍后重试即可的瞬时态）。原先 `/api/sessions` 那句既没重试也没吞 stderr，在 `set -e` 下命令替换赋值失败会**静默退出**——日志里只剩一行 `curl: (22) … 503`，整台节点被判失败，而发布脚本会整轮重试并每次再杀一遍 ChatGPT.desktop，永远收敛不了。三处探针统一收口到 `agent_api_retry`：503 与连不上按 deadline 重试，其它码立刻停，最终失败时打印 HTTP 码与响应体；新增 `tests/migrate-agent-retry_test.sh`（已接入 verify.sh）覆盖「200 直通 / 503→200 自愈 / 400 立刻停 / 连不上重试到上限 / 一直 503」。
- 修复 **Codex 会话列表全舰队 503**（`/api/sessions?assistant=codex` 稳定返回 `appserver_unavailable`，而 `assistant=claude` 正常）：DSH 重构引入 `routingChatBackend` 分派层后，装配点 `newAgentChatBackend()` 恒定返回 router，但 router 只实现了 `chatBackend`，漏掉 Codex 的会话目录接口；三处调用点都是对 `agentChatBackend` 做类型断言（`codexThreadCatalog` / `codexThreadManager` / `codexThreadReader`），断言必然失败，于是会话列表、归档改名与 `cwdForSession` 项目解析全部不可用，而聊天功能照常——只跑聊天测试发现不了。router 现在显式转发这三个接口，未装配 Codex 后端时仍给稳定的 `errAppServerUnavailable`；新增 `TestAgentChatBackendSatisfiesCodexCatalogInterfaces` **断言真实装配点**（负向对照：撤掉修复即精确失败）。
...
- 上传前先做同名预检：每个文件轮到自己时先向 agent 查一次目标目录清单（大小写不敏感，同名文件夹同样算冲突），命中就直接标「已存在，已跳过」并且**一个字节都不发**，避免 190 MB 传完才被 agent 的 `O_EXCL` 顶回 409；跳过的行留在面板里并提示「先在文件页删除它，再重新选择」，此时面板不会自动收起。拿不到目录清单（agent 不可达）时照旧上传，仍由服务端 409 兜底。
...
- 修复 shared 迁移在进程较多的 Mac 上查询 Desktop PID 时因 `awk` 提前退出触发 SIGPIPE（141），导致发布验收中止；查询现在消费完整进程列表。

- 修复 Codex 混合历史会话翻页失败：`thread/items/list` 的不支持响应仅影响当前会话，不再全局缓存并把其他会话的 items 游标误传给 turns 接口；保留 turns 游标分页，并补充跨会话回归测试。

## 2026-09-05

- 修复 shared Codex 会话在刷新、重新打开或浏览器切回前台后，把 Fleet 发起的运行中任务误判为 Desktop 占用：恢复会话时按内嵌历史的旧到新顺序选择最后一轮，分页回退仍使用最新优先顺序，保留正确的任务归属和后续消息控制权；补充两种历史顺序及 Fleet/Desktop 归属的回归测试。
...
- keeper 同时提供权限为 `0600` 的本机 Unix→loopback WebSocket 透明桥；因此四台现有节点可保留已经 Developer ID 签名并公证的 `a8d3d5d` agent，只迁移 plist/keeper/Desktop 启动环境就能进入同一个 app-server。新版 agent 仍原生支持 loopback WS，待新的 Apple 公证凭据恢复后再按正式发布流程替换二进制。
