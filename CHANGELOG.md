...
### 修复 iOS 聊天底部与键盘错位
- **移除错误延伸**：删除把聊天窗口推到根视口之外的负 `bottom`，输入框不再被 iOS 裁掉；屏幕底部由聊天根背景自然连续，不增加遮罩、伪元素或补色层。
...
- **缓存升级**：dashboard 外壳缓存升级到 v93，确保已安装 PWA 立即获取修复。

### 修复桌面端首屏误用移动布局
- **设备与宽度共同判定**：移动外壳不再只看 `860px` 断点，改为同时要求窄视口和粗指针触控输入；Mac 浏览器恢复窗口时即使短暂报告旧宽度，也不会误进移动模式。
- **前后状态一致**：CSS 响应式规则与 JavaScript 的移动端判断共用相同媒体条件，终端池、输入坞、手势和外壳不会再出现桌面/移动混合状态。
- **缓存升级**：dashboard 外壳缓存升级到 v92，确保浏览器立即获取新的断点规则与脚本。
