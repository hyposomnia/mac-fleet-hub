...
- `writerOwner`：优先从真实 writer lock 持锁进程判定 `fleet | desktop | 空`；直接 `codex exec resume` 不创建该 lock，因此还要检查当前 rollout 的打开进程，Fleet 私有 sidecar 仅作 reader 忽略，任何非 Fleet 活 holder 都视为外部 writer。不能依赖某个固定 Desktop 命令行形态。`thread/start` 创建的新 Fleet thread 在首轮运行期间也没有 lock 文件，此时仅接受本 fleet-agent 当前进程内 `loadedThreads + writerOwner=fleet + turnOwner=fleet + 同 turn id` 的组合租约。
...
- 外部 Desktop/CLI 已被服务端观察为 owner、但随后真实锁与非 Fleet rollout holder 均不存在时，立即清理内存 owner 并广播 idle；详情快照、只读 resume、connected sync、会话列表和队列 worker 都必须独立执行同一核对，避免重启或重连再次从陈旧 rollout 复活幽灵 writer。当前 agent 自己启动且仍加载的同 turn Fleet 租约不适用这条清理规则，因为 fresh thread 首轮没有 lock 文件。
...
- isolated 模式下 active rollout 无真实 lock holder、无非 Fleet rollout holder、也无当前 agent 的同 turn Fleet 租约时，Control、Resume、connected sync、会话列表均投影 idle，Input 可重新取得 writer；同 epoch Fleet 租约无 lock 时仍投影 running 并阻止第二个 turn；直接 CLI 仅持 rollout 时仍投影 external；`lsof`/进程分类失败时保守判外部占用。
