...
const appSandbox = {
  ...
  Date: FixedAppDate,
  // app.js 的能力探测用 AbortController + setTimeout 给自己上超时；沙箱里补齐这三个全局
  AbortController,
  setTimeout,
  clearTimeout,
};
...
// 回归背景（2026-09-22 真机）：mac3 的 mesh 到网关断了，`/m3/api/info` 要等满 nginx 的
// 60s 连接超时；能力探测没有超时，于是 DSH tab 在页面加载后 60s 内一直是初始隐藏态，
// 用户看到的就是「DeepSeek 没了」。探测必须自带超时，单台不可达不能拖住整体显隐。
test('assistant capability probe is bounded so one unreachable Mac cannot hide the DeepSeek tab', () => {
  assert.match(appSrc, /const CHAT_INFO_PROBE_TIMEOUT_MS = 5000;/);
  assert.match(appSrc, /async function probeAssistantInfo\(macId, timeoutMs = CHAT_INFO_PROBE_TIMEOUT_MS\)/);
  assert.match(appSrc, /const controller = new AbortController\(\);/);
  assert.match(appSrc, /const timer = setTimeout\(\(\) => controller\.abort\(\), timeoutMs\);/);
  assert.match(appSrc, /api\(macId, 'info', \{ signal: controller\.signal \}\)/);
  assert.match(appSrc, /finally \{\s*clearTimeout\(timer\);/);
  assert.match(appSrc, /probed\[m\.id\] = await probeAssistantInfo\(m\.id\);/);
  // 只有整份探测都结束才整体替换能力表，失败设备仍按「没有能力」处理（策略不变）
  assert.match(appSrc, /state\.assistantInfo = probed;\s*\n\s*syncAssistantTabs\(\);/);
});
...
test('Sub Agent strip sits above the composer and supports detail and list drawers', () => {
  const strip = indexHTML.indexOf('id="chat-subagents"');
  const composer = indexHTML.indexOf('class="chat-composer-inner"');
  assert.ok(strip > 0 && strip < composer);
  for (const id of ['chat-subagent-chips', 'chat-subagent-toggle', 'chat-subagent-drawer', 'chat-subagent-list', 'chat-subagent-detail', 'chat-subagent-detail-scroll']) {
    assert.match(indexHTML, new RegExp(`id="${id}"`));
  }
  assert.match(styleCSS, /\.chat-subagent-chips\s*\{[^}]*overflow-x:\s*auto;/s);
  const subagentsLayer = Number(styleCSS.match(/\.chat-subagents\s*\{[^}]*z-index:\s*(\d+)/s)?.[1]);
  const composerLayer = Number(styleCSS.match(/\.chat-composer-inner\s*\{[^}]*z-index:\s*(\d+)/s)?.[1]);
  assert.ok(composerLayer > subagentsLayer, '模型菜单所在的输入框应盖过 sub-agent 栏');
  assert.match(styleCSS, /\.chat-subagents\[data-mode="detail"\] \.chat-subagent-drawer\s*\{[^}]*50dvh/s);
  assert.match(styleCSS, /\.chat-subagents\[data-mode="list"\] \.chat-subagent-drawer\s*\{[^}]*34dvh/s);
  assert.match(styleCSS, /\.chat-subagents\[data-mode="detail"\] \.chat-subagent-drawer,[\s\S]*?68dvh/s);
  assert.match(appSrc, /chat\/subagents\?assistant=codex&sessionId=/);
  assert.match(appSrc, /chat\/history\?assistant=codex&sessionId=\$\{encodeURIComponent\(agent\.threadId\)\}/);
...
test('Sub Agent statuses reuse the existing agent dot vocabulary', () => {
  ...
  assert.deepEqual(values, [
    ...
    { key: 'unknown', label: '状态未知', dot: 'off' },
  ]);
  assert.equal(chatSubagentMeta({ role: 'explorer', nickname: 'Russell' }), 'explorer · Russell');
});

test('Completed Sub Agents disappear while unfinished agents remain available', () => {
  const agents = [
    { threadId: 'one', name: 'running', status: 'running' },
    { threadId: 'two', name: 'completed', status: 'completed' },
    { threadId: 'three', name: 'done', status: 'done' },
    { threadId: 'four', name: 'failed', status: 'failed' },
    { threadId: 'five', name: 'interrupted', status: 'interrupted' },
    { threadId: 'six', name: 'loading', status: 'notLoaded' },
    { threadId: '', name: 'invalid', status: 'running' },
  ];
  assert.deepEqual(Array.from(agents.filter(chatSubagentShouldShow), (agent) => agent.threadId), ['one', 'four', 'five', 'six']);
});

test('Sub Agent detail refreshes while running and once more on terminal transition', () => {
  assert.equal(chatSubagentShouldRefreshDetail('unknown', 'completed', false), true);
  assert.equal(chatSubagentShouldRefreshDetail('running', 'running', true), true);
  assert.equal(chatSubagentShouldRefreshDetail('running', 'completed', true), true);
  assert.equal(chatSubagentShouldRefreshDetail('completed', 'completed', true), false);
  assert.match(appSrc, /detail\.reloadAfterLoad = true/);
});

test('Sub Agent activity keeps identity in the model but remains internal to the main transcript', () => {
  const model = reduceChatEvent(createChatState(), {
    type: 'tool_update', itemId: 'sub-1', turnId: 'turn-1',
    data: {
      kind: 'subAgentActivity', internal: true, activityKind: 'started',
      agentThreadId: 'child-1', agentPath: '/root/review', status: 'completed',
    },
  });
  const item = model.items['sub-1'];
  assert.equal(item.internal, true);
  assert.equal(item.agentThreadId, 'child-1');
  assert.equal(item.agentPath, '/root/review');
  assert.equal(item.activityKind, 'started');
  assert.equal(isInternalChatTool(item), true);
});
...
test('pending Codex chat resolves the home directory once and migrates to the real thread', async () => {
  ...
  appSandbox.fetch = async (url, options = {}) => {
    ...
    if (url.endsWith('/api/info')) {
      data = { fileRoot: '/Users/test' };
    } else if (url.endsWith('/api/chat/start')) {
      data = {
          sessionId: 'thread-real', cwd: '/Users/test', approvalMode: 'on-request',
          models: [], model: '', effort: '', serviceTier: '',
          control: {
            serverEpoch: 'epoch-a', snapshotVersion: 1, accessMode: 'read_write', approvalMode: 'on-request',
            writerOwner: 'fleet', turnPhase: 'idle', items: [],
          },
        };
    } else {
      data = {
        serverEpoch: 'epoch-a', snapshotVersion: 2, accessMode: 'read_write', approvalMode: 'full-access',
        writerOwner: 'fleet', turnPhase: 'idle', items: [],
      };
    }
  ...
  try {
    ...
    assert.deepEqual(calls.map((call) => call.url), ['/m1/api/info', '/m1/api/chat/start', '/m1/api/chat/settings']);
    assert.equal(JSON.parse(calls[1].options.body).cwd, '/Users/test');
    assert.equal(JSON.parse(calls[2].options.body).approvalMode, 'full-access');
    ...
    assert.equal(chat.approvalConfirmedMode, 'full-access');
    assert.equal(chat.controlReady, true);
...
test('session project grouping follows Desktop assignments and merges projectless chats', () => {
  assert.equal(typeof sessionProjectInfo, 'function');
  assert.equal(typeof groupSessionsByProject, 'function');
  const sessions = [
    { sessionId: 'main', cwd: '/repos/get_job_done', projectId: 'jobs', projectName: 'get_job_done', projectCwd: '/repos/get_job_done' },
    { sessionId: 'worktree', cwd: '/Users/test/.codex/worktrees/6dd7/get_job_done', projectId: 'jobs', projectName: 'get_job_done', projectCwd: '/repos/get_job_done' },
    { sessionId: 'quick-a', cwd: '/Users/test/Documents/Codex/2026-08-05/qu', projectless: true },
    { sessionId: 'quick-b', cwd: '/Users/test/Documents/Codex/2026-08-05/zai', projectless: true },
  ];
  const groups = groupSessionsByProject(sessions);
  assert.equal(groups.length, 2);
  assert.deepEqual(Array.from(groups[0].arr, (session) => session.sessionId), ['main', 'worktree']);
  assert.equal(groups[0].name, 'get_job_done');
  assert.equal(groups[0].cwd, '/repos/get_job_done');
  assert.equal(groups[0].projectless, false);
  assert.deepEqual(Array.from(groups[1].arr, (session) => session.sessionId), ['quick-a', 'quick-b']);
  assert.equal(groups[1].name, '无项目');
  assert.equal(groups[1].cwd, '');
  assert.equal(groups[1].projectless, true);
...
test('Fleet retains release and read-only controls for actionable states', () => {
  ...
  assert.match(appSrc, /if \(chat\?\.accessMode === 'read_only'\) return 'readonly'/);
  assert.match(styleCSS, /\.chat-writer-state/);
  assert.match(styleCSS, /\.chat-release-writer/);
});

test('Dashboard is a server-authoritative chat shell with no local delivery state machine', () => {
  assert.doesNotMatch(appSrc, /macfleet:codex-followups/);
  assert.doesNotMatch(appSrc, /function flushChatFollowups/);
...
test('server queue states map to distinct native UI treatments and actions', () => {
  assert.match(appSrc, /writer_confirmation_required/);
  ...
  assert.match(appSrc, /恢复 Fleet 写入/);
  assert.match(appSrc, /decideServerChatQueue\(item, 'steer'\)/);
  const waitingTurnBranch = appSrc.slice(appSrc.indexOf('function renderChatFollowups'), appSrc.indexOf('function renderChatAttachments'));
  assert.doesNotMatch(waitingTurnBranch, /强制接管/);
});

test('every server queue state has an explicit placement, label, and legal browser actions', () => {
  const expected = {
    queued: ['transcript', ['cancel']],
    steering: ['transcript', []],
    waiting_turn: ['followup', ['steer', 'cancel']],
    writer_confirmation_required: ['transcript', ['wait', 'force', 'cancel']],
    waiting_writer: ['transcript', ['force', 'cancel']],
    waiting_access: ['transcript', ['enable-write', 'cancel']],
    takeover_check: ['transcript', []],
    takeover_confirmation_required: ['transcript', ['confirm-force', 'wait', 'cancel']],
    taking_over: ['transcript', []],
    sending: ['transcript', []],
    recovering: ['transcript', []],
    uncertain: ['transcript', ['retry', 'cancel']],
    failed: ['transcript', ['retry', 'cancel']],
    sent: ['hidden', []],
    cancelled: ['hidden', []],
  };
  for (const [status, [placement, actions]] of Object.entries(expected)) {
    const presentation = chatQueuePresentation({ status, allowedActions: actions });
    assert.equal(presentation.placement, placement, status);
    assert.ok(presentation.label.length > 0, `${status} has label`);
    assert.deepEqual(Array.from(presentation.actions), actions, status);
  }
  const unknown = chatQueuePresentation({ status: 'future_state' });
  assert.equal(unknown.placement, 'transcript');
  assert.match(unknown.label, /未知队列状态/);
  assert.deepEqual(Array.from(unknown.actions), []);
});

test('queue transcript cards render exactly the actions permitted by the state matrix', () => {
  const cases = {
    queued: ['消息已保存', '取消'],
    steering: ['正在插入当前任务'],
    writer_confirmation_required: ['等待并发送', '强制接管', '取消'],
    waiting_writer: ['强制接管', '取消'],
    waiting_access: ['恢复 Fleet 写入', '取消'],
    takeover_confirmation_required: ['中断全部并接管', '继续排队', '取消'],
    sending: ['正在启动下一轮'],
    recovering: ['正在核对'],
    uncertain: ['重新尝试', '取消'],
    failed: ['发送失败', '重新尝试', '取消'],
  ...
  for (const [status, labels] of Object.entries(cases)) {
    const allowedActions = ({
      queued: ['cancel'], writer_confirmation_required: ['wait', 'force', 'cancel'],
      waiting_writer: ['force', 'cancel'], waiting_access: ['enable-write', 'cancel'],
      takeover_confirmation_required: ['confirm-force', 'wait', 'cancel'], uncertain: ['retry', 'cancel'], failed: ['retry', 'cancel'],
    })[status] || [];
    const item = { status, affected: [], allowedActions };
    const text = nodeText(queueStatusCard(item));
    for (const label of labels) assert.match(text, new RegExp(label), `${status}: ${label}`);
    if (!chatQueuePresentation(item).actions.some((action) => action === 'force' || action === 'confirm-force')) {
      assert.doesNotMatch(text, /强制接管/, `${status} must not offer takeover`);
    }
  }
});
...
test('browser queue projection is replaced from the server without legacy persistence', () => {
  assert.doesNotMatch(appSrc, /macfleet:codex-followups/);
...
test('browser applies only newer complete server control snapshots', () => {
  ...
  assert.equal(isCompleteChatControlSnapshot({ serverEpoch: 'epoch-a', snapshotVersion: 1, accessMode: 'read_write', approvalMode: 'on-request', turnPhase: 'idle', items: [] }), true);
  assert.equal(isCompleteChatControlSnapshot({ serverEpoch: 'epoch-a', snapshotVersion: 1, accessMode: 'read_write', approvalMode: 'on-request', turnPhase: 'idle', items: [{ id: 'q1' }] }), false);
...
test('control projection clears request locks on a newer server item version', () => {
  const chat = {
    controlEpoch: 'epoch-a', controlVersion: 1, controlAppliedRequestSeq: 1,
    controlReady: true, followups: [{ id: 'q1', stateVersion: 1, decisionPending: 'wait' }],
    model: createChatState(), approvalMode: 'on-request', approvalConfirmedMode: 'on-request',
  };
  const applied = applyChatControlSnapshot(chat, {
    serverEpoch: 'epoch-a', snapshotVersion: 2, accessMode: 'read_write', approvalMode: 'on-request',
    writerOwner: 'desktop', turnPhase: 'idle', items: [{ id: 'q1', status: 'waiting_writer', stateVersion: 2, allowedActions: ['force', 'cancel'] }],
  }, 2);
  assert.equal(applied, true);
  assert.equal(chat.followups[0].decisionPending, '');
});

test('expired control transport disables mutations without inventing server state', () => {
  const chat = {
    controlReady: true, controlLastSyncedAt: fixedAppNowMs - 10_001,
    accessMode: 'read_write', writerOwner: 'fleet', turnPhase: 'running', turnOwner: 'fleet',
  };
  assert.equal(isChatControlFresh(chat, fixedAppNowMs), false);
  assert.equal(markChatControlSyncFailure(chat, fixedAppNowMs), true);
  assert.equal(chat.controlReady, false);
  assert.equal(chat.controlStale, true);
  assert.equal(chat.accessMode, 'read_write');
  assert.equal(chat.writerOwner, 'fleet');
  assert.equal(chat.turnPhase, 'running');
});
...
test('refresh restores only actionable queue items, not sent or cancelled history', () => {
  assert.match(appSrc, /filter\(\(item\) => chatQueuePresentation\(item\)\.placement !== 'hidden'\)/);
...
test('takeover is offered only for an external writer, never the current Fleet turn', () => {
  assert.deepEqual(Array.from(chatQueuePresentation({ status: 'writer_confirmation_required', allowedActions: ['wait', 'force', 'cancel'] }).actions), ['wait', 'force', 'cancel']);
  assert.deepEqual(Array.from(chatQueuePresentation({ status: 'waiting_writer', allowedActions: ['force', 'cancel'] }).actions), ['force', 'cancel']);
  assert.deepEqual(Array.from(chatQueuePresentation({ status: 'waiting_turn', allowedActions: ['steer', 'cancel'] }).actions), ['steer', 'cancel']);
  assert.deepEqual(Array.from(chatQueuePresentation({ status: 'queued', allowedActions: ['cancel'] }).actions), ['cancel']);
  assert.deepEqual(Array.from(chatQueuePresentation({ status: 'queued', allowedActions: ['cancel', 'future-action'] }).actions), ['cancel']);
  assert.match(appSrc, /chatQueuePresentation\(item\)\.actions\.includes\(action\)/);
});
...
test('composer queues non-empty running input and only stops when empty', () => {
  ...
  const readOnly = { controlReady: true, accessMode: 'read_only', model: { phase: 'idle' } };
...
test('chat image attachments open through one accessible fullscreen viewer with fit, zoom, and download controls', () => {
  ...
  assert.match(indexHTML, /id="chat-image-viewer-stage"/);
  assert.match(indexHTML, /id="chat-image-viewer-zoom-out"[^>]*aria-label="缩小图片"/);
  assert.match(indexHTML, /id="chat-image-viewer-zoom-reset"[^>]*aria-label="适应窗口"/);
  assert.match(indexHTML, /id="chat-image-viewer-zoom-in"[^>]*aria-label="放大图片"/);
  assert.match(indexHTML, /<a id="chat-image-viewer-download"[^>]*download[^>]*aria-label="下载图片"/);
  assert.match(styleCSS, /\.chat-image-viewer\s*\{[^}]*position:\s*fixed;[^}]*inset:\s*0;[^}]*z-index:\s*160;[^}]*padding:\s*0;/s);
  assert.match(styleCSS, /\.chat-image-viewer-canvas > img\s*\{[^}]*width:\s*auto;[^}]*height:\s*auto;[^}]*max-width:\s*100%;[^}]*max-height:\s*100%;[^}]*object-fit:\s*contain;/s);
  assert.doesNotMatch(styleCSS, /\.chat-image-viewer(?:-canvas)? > img\s*\{[^}]*(?<!max-)width:\s*100%;[^}]*(?<!max-)height:\s*100%;/s);
  assert.match(styleCSS, /\.chat-image-viewer-stage\s*\{[^}]*overflow:\s*auto;/s);
  ...
  assert.match(appSrc, /if \(handleChatImageViewerKeydown\(e\)\) return;/);
...
test('chat image viewer keeps zoom bounded and derives a useful download filename', () => {
  assert.equal(typeof clampChatImageViewerZoom, 'function');
  assert.equal(typeof chatImageDownloadName, 'function');
  assert.equal(typeof chatImageViewerFitScale, 'function');
  assert.equal(typeof chatImageViewerWheelZoom, 'function');
  assert.equal(typeof chatImageViewerPinchZoom, 'function');
  assert.equal(typeof chatImageViewerViewportAnchor, 'function');
  assert.equal(typeof chatImageViewerViewportCenter, 'function');
  assert.equal(clampChatImageViewerZoom(0.1), 0.5);
  assert.equal(clampChatImageViewerZoom(1.25), 1.25);
  assert.equal(clampChatImageViewerZoom(9), 4);
  assert.equal(clampChatImageViewerZoom(Number.NaN), 1);
  assert.ok(chatImageViewerWheelZoom(1, -10) > 1);
  assert.ok(chatImageViewerWheelZoom(1, -10) < 1.1);
  assert.equal(chatImageViewerPinchZoom(1, 100, 200), 2);
  assert.equal(chatImageViewerFitScale(1080, 2400, 1200, 800), 1 / 3);
  assert.equal(chatImageViewerFitScale(2400, 1080, 800, 1200), 1 / 3);
  assert.equal(chatImageViewerFitScale(320, 240, 1200, 800), 1);
  const center = chatImageViewerViewportCenter(
    { clientWidth: 800, clientHeight: 400, scrollWidth: 800, scrollHeight: 1600, scrollLeft: 0, scrollTop: 0 },
    { clientWidth: 400, clientHeight: 800, scrollWidth: 800, scrollHeight: 1600, scrollLeft: 200, scrollTop: 400 },
  );
  assert.equal(center.x, 0.5);
  assert.equal(center.y, 0.5);
  const liveCenter = chatImageViewerViewportCenter(
    { clientWidth: 400, clientHeight: 800, scrollWidth: 800, scrollHeight: 1600, scrollLeft: 100, scrollTop: 200 },
    { clientWidth: 400, clientHeight: 800, scrollWidth: 800, scrollHeight: 1600, scrollLeft: 0, scrollTop: 0 },
  );
  assert.equal(liveCenter.x, 0.375);
  assert.equal(liveCenter.y, 0.375);
  const focalAnchor = chatImageViewerViewportAnchor(
    {
      clientWidth: 400, clientHeight: 300, scrollWidth: 800, scrollHeight: 600,
      scrollLeft: 200, scrollTop: 150,
      getBoundingClientRect: () => ({ left: 20, top: 30 }),
    },
    { clientWidth: 400, clientHeight: 300, scrollWidth: 800, scrollHeight: 600, scrollLeft: 200, scrollTop: 150 },
    { focalClientX: 120, focalClientY: 105 },
  );
  assert.equal(focalAnchor.x, 0.375);
  assert.equal(focalAnchor.y, 0.375);
  assert.equal(focalAnchor.viewportX, 100);
  assert.equal(focalAnchor.viewportY, 75);
  assert.equal(
    chatImageDownloadName('/m2/api/chat/media?path=%2FUsers%2Ftest%2Fcaptures%2Fscreen.png', '截图'),
    'screen.png',
  );
  assert.equal(chatImageDownloadName('blob:shot', 'shot.png'), 'shot.png');
  assert.equal(chatImageDownloadName('https://example.test/assets/result.webp?raw=1', ''), 'result.webp');
});

test('opening the chat image viewer resets zoom, prepares download, and restores focus on close', () => {
  assert.equal(typeof openChatImageViewer, 'function');
  assert.equal(typeof closeChatImageViewer, 'function');
  assert.equal(typeof setChatImageViewerZoom, 'function');
  const elements = Object.fromEntries([
    'chat-image-viewer', 'chat-image-viewer-image', 'chat-image-viewer-stage', 'chat-image-viewer-canvas',
  ...
  ].map((id) => [`#${id}`, testElement(id === 'chat-image-viewer-download' ? 'a' : 'div')]));
  const viewer = elements['#chat-image-viewer'];
  const image = elements['#chat-image-viewer-image'];
  const download = elements['#chat-image-viewer-download'];
  const reset = elements['#chat-image-viewer-zoom-reset'];
  const trigger = testElement('button');
  viewer.hidden = true;
  image.complete = false;
  trigger.isConnected = true;
  trigger.closest = () => trigger;
  const previousQuerySelector = appSandbox.document.querySelector;
  const previousActiveElement = appSandbox.document.activeElement;
  appSandbox.document.querySelector = (selector) => elements[selector] || null;
  for (const control of [...Object.values(elements), trigger]) {
    control.focus = function focus() {
      this.focused = true;
      appSandbox.document.activeElement = this;
    };
  }
  try {
    setChatImageViewerZoom(2);
    openChatImageViewer('blob:shot', 'shot.png', trigger);
    assert.equal(viewer.hidden, false);
    assert.equal(image.src, 'blob:shot');
    assert.equal(image.alt, 'shot.png');
    assert.equal(reset.textContent, '100%');
    assert.equal(download.getAttribute('href'), 'blob:shot');
    assert.equal(download.getAttribute('download'), 'shot.png');
    assert.equal(elements['#chat-image-viewer-close'].focused, true);
    let prevented = false;
    assert.equal(handleChatImageViewerKeydown({ key: '+', preventDefault() { prevented = true; } }), true);
    assert.equal(prevented, true);
    assert.equal(reset.textContent, '125%');
    appSandbox.document.activeElement = download;
    assert.equal(handleChatImageViewerKeydown({ key: 'Tab', shiftKey: false, preventDefault() {} }), true);
    assert.equal(appSandbox.document.activeElement, elements['#chat-image-viewer-close']);
    assert.equal(handleChatImageViewerKeydown({ key: 'Tab', shiftKey: true, preventDefault() {} }), true);
    assert.equal(appSandbox.document.activeElement, download);
    setChatImageViewerZoom(1.5);
    ...
    appSandbox.document.querySelector = previousQuerySelector;
    appSandbox.document.activeElement = previousActiveElement;
  }
});
