...
const CUSTOM_ACCESS_KEY_PROJECT = '__custom__';
function accessKeyProjectPath() {
  const project = $('#automation-key-project');
  return (project.value === CUSTOM_ACCESS_KEY_PROJECT
    ? $('#automation-key-project-custom').value : project.value).trim();
}
function updateAccessKeyBindingFields() {
  const device = $('#automation-key-device').value;
  const client = $('#automation-key-client');
  const project = $('#automation-key-project');
  const custom = $('#automation-key-project-custom');
  const session = $('#automation-key-session');
  client.disabled = !device;
  project.disabled = !device || !client.value;
  custom.hidden = project.value !== CUSTOM_ACCESS_KEY_PROJECT;
  custom.disabled = project.disabled || custom.hidden;
  session.disabled = project.disabled || !accessKeyProjectPath();
}
function renderAccessKeyProjectOptions() {
  const project = $('#automation-key-project');
  const custom = $('#automation-key-project-custom');
  const selected = accessKeyProjectPath();
  const wasCustom = project.value === CUSTOM_ACCESS_KEY_PROJECT;
  ...
  project.replaceChildren(h('option', { value: '', text: '全部项目' }));
  for (const [path, name] of projects) project.append(h('option', { value: path, text: `${name} · ${path}` }));
  project.append(h('option', { value: CUSTOM_ACCESS_KEY_PROJECT, text: '手动输入项目路径…' }));
  if (selected && projects.has(selected)) {
    project.value = selected;
    custom.value = '';
  } else if (selected || wasCustom) {
    project.value = CUSTOM_ACCESS_KEY_PROJECT;
...
async function loadAccessKeyBindingSessions() {
  ...
  try {
    ...
    const count = renderAccessKeyProjectOptions();
    status.textContent = count ? '' : '没有已发现的项目，可手动输入项目路径。';
    renderAccessKeySessionOptions();
  } catch (error) {
    if (sequence === automationBindingLoadSeq) status.textContent = `读取候选失败：${error.message}。可手动输入项目路径。`;
  }
}
function renderAccessKeySessionOptions(selected = $('#automation-key-session').value) {
  const root = $('#automation-key-session');
  root.replaceChildren(h('option', { value: '', text: '全部会话' }));
  const project = accessKeyProjectPath();
  const seen = new Set();
  for (const item of automationBindingSessions) {
    if ((item.projectCwd || item.cwd) === project && item.sessionId && !seen.has(item.sessionId)) {
      seen.add(item.sessionId);
      root.append(h('option', { value: item.sessionId, text: item.title ? `${item.title} · ${item.sessionId}` : item.sessionId }));
    }
  }
  if (selected && !seen.has(selected)) root.append(h('option', { value: selected, text: `${selected}（当前不在列表）` }));
  root.value = selected;
}
function openAccessKeyForm(key = null) {
  ...
  const projectPath = key?.binding?.project_path || '';
  $('#automation-key-project').value = projectPath ? CUSTOM_ACCESS_KEY_PROJECT : '';
  $('#automation-key-project-custom').value = projectPath;
  automationBindingSessions = [];
  renderAccessKeyProjectOptions();
  renderAccessKeySessionOptions(key?.binding?.session_id || '');
  updateAccessKeyBindingFields();
...
function chatOwnershipPresentation(chat) {
  if (!chat) return null;
  if (!chat.controlReady) {
    return chat.controlStale
...
function renderChatOwnershipHead(chat = state.chat) {
  ...
  const action = presentation.action === 'release' ? releaseChatWriter
    : (presentation.action === 'enable-write' ? enableChatWriter : null);
  tt.append(h('span', { class: `chat-writer-state${presentation.className ? ` ${presentation.className}` : ''}` },
    h('span', { text: presentation.text }),
    action ? h('button', {
      type: 'button', class: 'chat-release-writer', text: presentation.actionLabel,
      disabled: chat.releasingWriter || chat.changingAccess ? '' : null,
      onclick: action,
    }) : null));
}
...
const CHAT_CONTROL_STALE_MS = 10000;

function isChatControlFresh(chat, now = Date.now()) {
  const syncedAt = Number(chat?.controlLastSyncedAt) || 0;
  return syncedAt > 0 && Math.max(0, Number(now) - syncedAt) <= CHAT_CONTROL_STALE_MS;
}

function markChatControlSyncFailure(chat, now = Date.now()) {
  if (!chat || chat.pendingStart || isChatControlFresh(chat, now)) return false;
  const changed = chat.controlReady || !chat.controlStale;
  chat.controlReady = false;
  chat.controlStale = true;
  return changed;
}
...
function isCompleteChatControlSnapshot(snapshot) {
  return !!snapshot
    ...
    && ['untrusted', 'on-request', 'full-access'].includes(snapshot.approvalMode)
    && ['running', 'idle', 'unknown'].includes(snapshot.turnPhase)
    && Array.isArray(snapshot.items)
    && snapshot.items.every((item) => Array.isArray(item?.allowedActions));
}
...
function applyChatControlSnapshot(chat, source, requestSeq = 0) {
  ...
  const visibleQueue = (Array.isArray(snapshot.items) ? snapshot.items : [])
    ...
    .map((item) => {
      const previous = currentQueue.get(item.id);
      return { ...item, decisionPending: previous?.stateVersion === item.stateVersion ? (previous.decisionPending || '') : '' };
    });
...
const CHAT_QUEUE_UI = Object.freeze({
  ...
  taking_over: { placement: 'transcript', label: '正在强制接管目标 Mac 上的会话…' },
  sending: { placement: 'transcript', label: '正在启动下一轮…' },
  recovering: { placement: 'transcript', label: '服务已重启，正在核对这条消息是否已经投递…' },
  uncertain: { placement: 'transcript', label: '上次投递结果未知。请先核对会话历史，再决定重试或取消。' },
  failed: { placement: 'transcript', label: '发送失败，请重新尝试' },
  sent: { placement: 'hidden', label: '已发送' },
  cancelled: { placement: 'hidden', label: '已取消' },
});

const CHAT_QUEUE_ACTIONS = new Set(['steer', 'wait', 'force', 'confirm-force', 'retry', 'cancel', 'enable-write']);

function chatQueuePresentation(item) {
  const status = item?.status || 'queued';
  const configured = CHAT_QUEUE_UI[status];
  if (!configured) return { status, placement: 'transcript', label: `未知队列状态：${status}`, actions: [] };
  return {
    status,
    placement: configured.placement,
    label: status === 'failed' && item?.error ? item.error : configured.label,
    actions: Array.isArray(item?.allowedActions) ? item.allowedActions.filter((action) => CHAT_QUEUE_ACTIONS.has(action)) : [],
  };
}

function isFleetQueueFollowup(item) {
  return chatQueuePresentation(item).placement === 'followup';
}
...
async function decideServerChatQueue(item, action) {
  ...
  try {
    ...
    if (action === 'cancel') chat.model = FleetChatModel.removeMessage(chat.model, item.clientMessageId);
...
function queueStatusCard(item) {
  const presentation = chatQueuePresentation(item);
  const status = presentation.status;
  const pending = !!item.decisionPending;
  const controlBlocked = chatControlActionsBlocked();
  const actions = [];
  for (const action of presentation.actions) {
    if (action === 'steer') continue;
    if (action === 'enable-write') {
      actions.push(h('button', { type: 'button', class: 'btn sm accent', disabled: chatAccessBusy() || controlBlocked ? '' : null, onclick: enableChatWriter }, chatAccessBusy() ? '恢复中…' : '恢复 Fleet 写入'));
      continue;
    }
    const danger = action === 'force' || action === 'confirm-force';
    const bare = action === 'cancel';
    const labels = {
      wait: status === 'writer_confirmation_required' ? '等待并发送' : '继续排队',
      force: pending && item.decisionPending === 'force' ? '正在接管…' : '强制接管',
      'confirm-force': pending ? '正在接管…' : '中断全部并接管',
      retry: pending ? '正在重试…' : '重新尝试',
      cancel: '取消',
    };
    actions.push(h('button', {
      type: 'button', class: `btn sm${danger ? ' danger' : ''}${bare ? ' bare' : ''}`,
      disabled: pending || controlBlocked ? '' : null, onclick: () => decideServerChatQueue(item, action),
    }, labels[action] || action));
  }
  return h('section', { class: `chat-queue-status ${status}` },
    h('div', { class: 'chat-queue-status-title', text: presentation.label }),
...
function updateCachedChatFromSession(macId, session) {
  if (!session?.sessionId) return;
  const chat = state.chatCache.get(chatCacheKey(macId, session.sessionId));
  updateChatUpdatedAt(chat, session.mtime);
  if (chat?.historyReady) session.waitingVisible = pendingChatRequestCount(chat) > 0;
}

function pendingChatRequestCount(chat) {
...
function sessionHasVisibleRequest(session) {
  ...
  const chat = state.chatCache.get(chatCacheKey(session.macId, session.sessionId));
  return !chat?.historyReady || pendingChatRequestCount(chat) > 0;
}
...
function requestActionButton(label, response, primary = false) {
  return h('button', {
    ...
    disabled: chatMutationsBlocked() ? '' : null,
...
function renderUserInputQuestion(item, question, index) {
  ...
  return h('fieldset', { class: 'chat-request-fieldset', disabled: chatMutationsBlocked() ? '' : null, dataset: { questionId: question.id, fieldName: name } },
...
function renderChatUserInputRequest(item) {
  ...
  h('div', { class: 'chat-approval-body' },
    ...
    h('div', { class: 'chat-approval-actions' },
      h('button', { type: 'submit', class: 'btn sm primary', disabled: chatMutationsBlocked() ? '' : null }, '提交回答'))));
...
function renderMcpFormField(key, schema, required) {
  ...
  if (options.length && schema.type === 'array') {
    control = h('div', { class: 'chat-request-options' }, options.map((option) => h('label', { class: 'chat-request-option' },
      h('input', { type: 'checkbox', name: key, value: option.value, disabled: chatMutationsBlocked() ? '' : null }),
      h('span', { text: option.label }))));
  } else if (options.length) {
    control = h('select', { class: 'chat-request-input', name: key, required: required ? '' : null, disabled: chatMutationsBlocked() ? '' : null },
    ...
    control = h('label', { class: 'chat-request-option inline' },
      h('input', { type: 'checkbox', name: key, checked: schema.default === true ? '' : null, disabled: chatMutationsBlocked() ? '' : null }),
...
function renderChatElicitationRequest(item) {
  ...
  if (!pending) body.push(h('div', { class: 'muted', text: '已发送响应。' }));
    ...
    body.push(h('div', { class: 'chat-approval-actions' },
      h('button', { type: 'submit', class: 'btn sm primary', disabled: chatMutationsBlocked() ? '' : null }, '提交'),
      requestActionButton('拒绝', { action: 'decline' })));
  }
  const form = h('form', {
    class: 'chat-approval chat-request',
...
function chatComposerAction(chat, hasContent) {
  if (chat?.starting || chat?.releasingWriter || chat?.changingAccess) return 'transition';
...
function chatMutationsBlocked(chat = state.chat) {
  return !chat || !chat.controlReady || chat.accessMode !== 'read_write' || chat.starting || chat.releasingWriter || chat.changingAccess;
}

function chatControlActionsBlocked(chat = state.chat) {
  return !chat || !chat.controlReady || chat.starting || chat.releasingWriter || chat.changingAccess;
}
...
function updateChatComposerState() {
  ...
  if (attach) attach.disabled = mutationBlocked;
  if (options) options.disabled = mutationBlocked;
  if (approval) approval.disabled = mutationBlocked || !state.chat?.approvalControlsEnabled;
  if (mutationBlocked) {
    closeChatSkillMenu();
    closeChatApproval();
    closeChatOptions();
  }
  send.disabled = !state.chat || !!state.chat.starting || mutationBlocked || action === 'wait-desktop' || (action === 'interrupt'
...
function renderChatFollowups() {
  ...
  if (!box) return;
  clear(box);
  const items = (chat?.followups || []).filter(isFleetQueueFollowup);
...
async function addChatFiles(files) {
  const chat = state.chat;
  if (chatMutationsBlocked(chat) || !files || !files.length) return;
  ...
  for (const file of Array.from(files)) {
    ...
    const att = {
      ...
      name: file.name, size: file.size, mime: file.type, previewUrl,
      uploading: !pendingUpload, pendingUpload, file: pendingUpload ? file : null,
    };
    chat.attachments.push(att);
    renderChatAttachments();
    if (!pendingUpload) uploadChatFile(chat, att, file);
  }
}
...
async function uploadChatFile(chat, att, file) {
  try {
    ...
    if (state.chatCache.get(chat.cacheKey) !== chat) return;
    Object.assign(att, saved, { uploading: false, pendingUpload: false, file: null, error: '' });
  } catch (e) {
    att.uploading = false;
...
async function openChatSession(s) {
  ...
  if (chat) {
    ...
    chat = {
      ...
      controlLastSyncedAt: s.pendingStart ? Date.now() : 0, controlStale: false,
  ...
  if (!Number.isFinite(chat.controlLastSyncedAt)) chat.controlLastSyncedAt = chat.pendingStart ? Date.now() : 0;
  if (typeof chat.controlStale !== 'boolean') chat.controlStale = false;
  if (typeof chat.approvalUpdatePending !== 'boolean') chat.approvalUpdatePending = false;
  if (!Number.isFinite(chat.approvalUpdateSeq)) chat.approvalUpdateSeq = 0;
  ...
  if (s.pendingStart) {
    chat.pendingStart = true;
    chat.unscoped = !!s.unscoped;
...
async function selectChatApprovalMode(value) {
  ...
  $('#chat-approval')?.focus();
  if (chat.pendingStart) {
    chat.approvalConfirmedMode = approvalMode;
    return;
  }
  const updateSeq = (Number(chat.approvalUpdateSeq) || 0) + 1;
  chat.approvalUpdateSeq = updateSeq;
  ...
  try {
    ...
    const latest = chat.approvalUpdateSeq === updateSeq;
    if (latest) chat.approvalUpdatePending = false;
    if (!isCompleteChatControlSnapshot(result?.control || result)) throw new Error('服务端返回了无效的权限状态');
    applyChatControlSnapshot(chat, result, controlRequestSeq);
    if (latest) {
    ...
    if (chat.approvalUpdateSeq === updateSeq) {
...
function configureChatOptions(chat, resumed) {
  ...
  chat.approvalUpdatePending = false;
  chat.approvalUpdateSeq = Number(chat.approvalUpdateSeq) || 0;
...
async function ensurePendingChatStarted(chat) {
  ...
  const task = (async () => {
    ...
    let cwd = chat.cwd;
    if (chat.unscoped) {
      const info = await api(chat.macId, 'info');
    ...
    if (!applyChatControlSnapshot(chat, started, controlRequestSeq)) throw new Error('服务端返回了无效的会话控制状态');
  ...
  chat.startPromise = task;
  try {
    return await task;
  } finally {
    if (chat.startPromise === task) chat.startPromise = null;
    chat.starting = false;
    if (chat.pendingStart) chat.loading = false;
    if (state.chat === chat) {
      renderChat();
      updateChatComposerState();
    }
...
async function submitChatInput({ deliveryMode = 'next' } = {}) {
  ...
  if (chat.pendingStart) {
    try {
      ...
      if (state.chat === chat) toast('会话初始化失败：' + e.message, 'err');
      return;
    }
    if (state.chat !== chat) return;
    raw = typeof input.value === 'string' ? input.value : '';
  }
  const deferredUploads = (chat.attachments || []).filter((att) => att.pendingUpload && att.file);
  if (deferredUploads.length) {
    for (const att of deferredUploads) att.uploading = true;
    renderChatAttachments();
    await Promise.all(deferredUploads.map((att) => uploadChatFile(chat, att, att.file)));
    if (state.chat !== chat) return;
  }
...
async function saveServerChatQueueItem(chat, item, deliveryMode) {
  try {
...
async function releaseChatWriter() {
  const chat = state.chat;
  const retryingStuckRelease = chat?.accessMode === 'read_only' && chat?.writerOwner === 'fleet';
  if (!chat || (chat.accessMode === 'read_only' && !retryingStuckRelease) || chat.releasingWriter || !chat.controlReady) return;
  chat.releasingWriter = true;
  renderChatOwnershipHead(chat);
  updateChatComposerState();
  try {
    const controlRequestSeq = beginChatControlRequest(chat);
...
function chatAccessBusy() {
  return !!state.chat?.changingAccess;
}

async function enableChatWriter() {
  const chat = state.chat;
  if (!chat || chat.accessMode !== 'read_only' || chat.writerOwner === 'fleet' || chat.changingAccess || !chat.controlReady) return;
  chat.changingAccess = true;
  renderChatOwnershipHead(chat);
  renderChat();
  updateChatComposerState();
  try {
...
async function respondChatRequest(requestId, response) {
  ...
  if (chatMutationsBlocked(chat) || !requestId || request?.submitting) return;
...
function init() {
  ...
  $('#automation-key-device').onchange = () => {
    ...
    $('#automation-key-project-custom').value = '';
  ...
  $('#automation-key-project').onchange = () => {
    $('#automation-key-project-custom').value = '';
    $('#automation-key-session').value = '';
    updateAccessKeyBindingFields();
    renderAccessKeySessionOptions();
    if (!$('#automation-key-project-custom').hidden) $('#automation-key-project-custom').focus();
  };
  $('#automation-key-project-custom').oninput = () => {
    $('#automation-key-session').value = '';
    updateAccessKeyBindingFields();
    renderAccessKeySessionOptions();
  };
