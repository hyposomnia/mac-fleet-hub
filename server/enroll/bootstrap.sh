...
#
# 流程：问域名/验证码 → 经网关 TOTP 校验换取一次性入网密钥（编号由网关自动分配）→ 下载安装包 → 安装并入网。
set -euo pipefail

bold() { printf "\033[1m%s\033[0m\n" "$1"; }
bold "== mac-fleet-hub 自助安装 =="

# 允许通过环境变量预填（curl|bash 时也可：DOMAIN=.. CODE=.. bash <(curl ...)）
...
OUT="$(curl -sS -w $'\n%{http_code}' -X POST "${BASE}/join" -H 'content-type: application/json' \
  -d "{\"code\":\"${CODE}\"}")" || { echo "✗ 网络不通，请检查域名/端口"; exit 1; }
HTTP="${OUT##*$'\n'}"
RESP="${OUT%$'\n'*}"
if [ "$HTTP" != "200" ]; then
...
# 编号由网关自动分配，从返回里取
IDX="$(printf '%s' "$RESP" | sed -n 's/.*"index":"\([^"]*\)".*/\1/p')"
[ -n "$AUTHKEY" ] || { echo "✗ 未取得入网密钥"; exit 1; }
[[ "$IDX" =~ ^[1-9][0-9]?$ ]] || { echo "✗ 网关未分配有效编号"; exit 1; }
echo "✓ 验证通过，网关已分配编号 mac${IDX}，并发一次性入网密钥"
