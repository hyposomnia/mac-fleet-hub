...
contains "$SETUP" 'CODEX_APPSERVER_SOCK="${CODEX_APPSERVER_SOCK:-ws://127.0.0.1:47682/rpc}"'
contains "$SETUP" 'CODEX_APPSERVER_LISTEN="ws://127.0.0.1:${CODEX_APPSERVER_PORT}"'
contains "$SETUP" 'launchctl setenv CODEX_APP_SERVER_WS_URL "$CODEX_APPSERVER_SOCK"'
...
contains "$MIGRATE" '/usr/bin/open --env "CODEX_APP_SERVER_WS_URL=$SHARED_ENDPOINT"'
contains "$MIGRATE" 'codexAppServerConnected'
