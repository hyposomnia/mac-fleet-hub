...
# 重试即可」的瞬时态。2026-09-16 发布时，其中一个探针（/api/sessions）既没重试也没
# 吞 stderr，在 `set -e` 下命令替换赋值失败**静默退出**——日志里只剩一行
# `curl: (22) The requested URL returned error: 503`，脚本整轮重试、每次再杀一遍
# Desktop，永远收敛不了。
#
# 本测试从真实脚本里抽出该助手（按函数名到行首 `}`），配桩 curl 验证四种行为：
# 一次成功 / 503 后转好 / 真错误立刻停 / 连不上按瞬时态重试。
set -euo pipefail
...
#!/usr/bin/env bash
# 桩 curl：忽略真实参数，按 ${MODE} 按次序吐「响应体\nHTTP码」；调用次数记在 ${COUNTER}。
...
printf 'agent_api_retry "$1" "http://127.0.0.1:7682/api/chat/skills"\n' >> "$TMP/runner.sh"

fails=0
check() { # mode attempts want_rc want_calls want_stderr label
  local mode="$1" attempts="$2" want_rc="$3" want_calls="$4" want_stderr="$5" label="$6"
  echo 0 > "$TMP/counter"
  local rc=0 out
  if out="$(COUNTER="$TMP/counter" MODE="$mode" PATH="$TMP/bin:$PATH" \
      /bin/bash "$TMP/runner.sh" "$attempts" 2>&1)"; then rc=0; else rc=$?; fi
...
# 一次成功：不重试。
check ok         3 0 1 "" "200 直接通过"
# 关键回归：503 是瞬时态，必须重试到成功，而不是判死。
check 503then200 3 0 2 "" "503 后转 200 自愈"
# 真错误不空转重试。
check 400        3 1 1 "http=400" "400 立刻停且带码与响应体"
# 连不上（curl 非零退出、无输出）同样按瞬时态重试，直到上限。
check noreply    2 1 2 "连不上" "连不上重试到上限后失败"
# 一直 503：重试到上限，并把 503 与响应体打出来供排障。
check always503  2 1 2 "http=503" "一直 503 重试到上限"
